...
During the **enemy's combat movement phase**, whenever enemy planes fly **over** (enter, including merely passing through) a territory containing your AA gun, roll 1 die per plane in the raid; each "1" shoots down one plane (attacker chooses which planes die in multi-plane raids? — the rule says "one plane is shot down and eliminated from play"; the parallel combat-phase rule p. 18 says "the attacker immediately removes the plane of his or her choice", use attacker-chooses). No counterattack. **Once an AA gun has fired it is no longer involved in the land combat** that turn (p. 13, 18). AA guns fire only during enemy combat movement / the AA step of combat — never during the defender's own combat (FAQ p. 1: "AA guns fire only during enemy combat movement"). Engine model: each AA gun fires **once at each enemy aircraft per turn**, tracked per-plane (by unit id). Aircraft flown in over several separate move actions are therefore each fired at (not just the first batch), while a plane already shot at during overflight is not shot at again in the in-battle AA step (§6.2). One die per plane; no cap on the number of planes.
